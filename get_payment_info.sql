DROP FUNCTION ROLE_REPORT(INTEGER, INTEGER);
...
-- 25. ABC_PAYMENT_DATE
    ...
    IF v_member_rec.id IS NOT NULL THEN
        ...
        RAISE NOTICE 'payment date %', ABC_PAYMENT_DATE;        
        IF ( ABC_PAYMENT_DATE IS NULL ) AND (v_member_rec.metadata like 'Transferred_from_member_id%') THEN
            RAISE NOTICE 'Transferred Member Process..';
            v_member_id := (REGEXP_MATCHES(v_member_rec.metadata, '[0-9]+'))[1]::integer;
            RAISE NOTICE 'v_member_id %', v_member_id;
            
            SELECT  scm.payment_method, scm.transaction_id, scm.club_joining_date, scm.created_at
            INTO STRICT v_member_rec1
            FROM "sy_club_members" scm
            WHERE id = v_member_id; 							

            ABC_PAYMENT_DATE := get_payment_info(v_member_rec1.payment_method, v_member_rec1.transaction_id);

            IF ( ABC_PAYMENT_DATE IS NULL ) THEN
                ABC_PAYMENT_DATE := v_member_rec1.club_joining_date;
                IF ( ABC_PAYMENT_DATE IS NULL ) THEN
                    ABC_PAYMENT_DATE := v_member_rec1.created_at;
                END IF;
            END IF;
        END IF;
    END IF;
...
--27.28 IS_ABC_RENEWED, ABC_MEMBERSHIP_STATUS
	...
	-- RAISE NOTICE '27.28 IS_ABC_RENEWED, ABC_MEMBERSHIP_STATUS % % %', v_is_renewed.event_registration_id, v_member_rec.event_registration_id, v_ABC_id;										   
	IF FOUND AND v_is_renewed.event_registration_id = v_member_rec.renewed_from THEN 
        IS_ABC_RENEWED := true;
    ELSE
